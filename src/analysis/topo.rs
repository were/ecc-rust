...
fn dfs_topology<'ctx>(
  ...
  if let InstOpcode::Branch(BranchMetadata::ReturnJump) = cur.last_inst().unwrap().get_opcode() {
    // If this branch is generated by a inlined return, we should not topological sort it.
  } else {
