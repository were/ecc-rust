...
use trinity::{
  ir::{
    value::{
      ...
      instruction::{
        BranchInst, InstructionRef, InstOpcode, CmpPred, BinaryOp, BinaryInst, BranchMetadata
      }
    },
...
fn dfs_topology<'ctx>(
  ...
  // If this branch is generated by a inlined return, we should not topological sort it with
  // a special rule.
  let is_return_jmp =
    &InstOpcode::Branch(BranchMetadata::ReturnJump) == cur.last_inst().unwrap().get_opcode();
  {
    ...
    for succ in successors.iter() {
      ...
      // We cannot go out this loop until we traverse all the blocks inside.
      let not_an_exit = loop_stack.iter().all(|x| {
        res.get_loop(*x).get_exit().get_skey() != dst_key
      }) || {
        is_return_jmp && loop_stack.is_empty()
      };
      if not_an_exit {
        if !visited[dst_key] {
